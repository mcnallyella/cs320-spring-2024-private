...
type lexpr
  ...
  let rec convert_expr_to_lexpr (e : expr) : lexpr =
    match e with
    | Num n -> Num n
    | Bool b -> Bool b
    | Unit -> Unit
    | Var id -> Var id
    | Uop (op, e1) -> Uop (op, convert_expr_to_lexpr e1)
    | Bop (op, e1, e2) -> Bop (op, convert_expr_to_lexpr e1, convert_expr_to_lexpr e2)
    | Ife (e1, e2, e3) -> Ife (convert_expr_to_lexpr e1, convert_expr_to_lexpr e2, convert_expr_to_lexpr e3)
    | Fun ([], body) -> convert_expr_to_lexpr body
    | Fun (arg::args, body) ->
        Fun (arg, convert_expr_to_lexpr (Fun (args, body)))
    | App (e1, e2) -> App (convert_expr_to_lexpr e1, convert_expr_to_lexpr e2)
    | Trace e1 -> Trace (convert_expr_to_lexpr e1)
    | Let (id, args, e1, e2) ->
      App (
        Fun (id, convert_expr_to_lexpr e2),
        convert_expr_to_lexpr e1
      )
...
let rec translate (e : lexpr) : stack_prog =  (* TODO *)
  ...
  | Var x -> [Lookup x]
  | Uop (op, e1) ->
    translate e1 @ [match op with Neg -> Sub | Not -> Trace] 
  | Bop (op, e1, e2) ->
    translate e1 @ translate e2 @ [match op with Add -> Add | Sub -> Sub | Mul -> Mul | Div -> Div | _ -> failwith "error"]
  | Ife (condition, e1, e2) ->
  ...
  | Trace e ->
    ...
    (* adds the stack programing specific words to each lexpr *)
    (* concats whole prog to string *)
let rec serialize (p : stack_prog) : string =  (* TODO *)
  let string_of_command c = match c with
  | Push const -> "push " ^ (match const with Num n -> string_of_int n | Bool b -> if b then "true" else "false" | Unit -> "unit")
  | Swap -> "swap"
  | Trace -> "trace"
  | Add -> "add"
  | Sub -> "sub"
  ...
  | Fun (x, p) -> "fun " ^ x ^ " begin " ^ serialize p ^ " end"
  | Call -> "call"
  | Return -> "return"
  | Assign x -> "assign " ^ x
  | Lookup x -> "lookup " ^ x
