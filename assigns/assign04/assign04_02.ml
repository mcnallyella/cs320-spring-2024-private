...
(* Outline: 
  ...
  in
    ...
    let rec loop_paths paths output_points =
      match paths with 
      | [] -> List.rev output_points
      | hd::tl ->  let (path, start) = hd in 
      let end_point = (find_endpoint (make_list len path) start true) in
        if end_point != (-10000)
          then loop_paths tl (end_point::output_points)
      else loop_paths tl output_points
