...
let rec eval_prog stack prog trace =  (* TODO *)
  match prog with
  | Drop::p ->
    (match stack with 
    | x::s -> eval_prog s p trace
    | [] -> eval_prog [] [] ("Panic"::trace))
  | Swap::p ->
    (match stack with 
    | x1::x2::s -> eval_prog (x2::x1::s) p trace
    | x1::[]::[] -> eval_prog [] [] ("Panic"::trace)
    | [] -> eval_prog [] [] ("Panic"::trace))
  | Dup::p ->                    (* dup *)
    (match stack with 
    | x::s -> eval_prog (x::x::s) p trace
    | [] -> eval_prog [] [] ("Panic"::trace))
  | Trace::p                 (* . *)
  | Add::p ->                    (* + *)
    (match stack with
    | x1::x2::s -> eval_prog (x1+x2)::s p trace
    | x1::[]::[] -> eval_prog [] [] ("Panic"::trace)
    | [] -> eval_prog [] [] ("Panic"::trace))
  | Sub::p ->                    (* - *)
    (match stack with 
  ...
  | Lt::p ->                     (* < *)
    (match stack with 
    | x1::x2::s -> 
      (if x1 < x2 then eval_prog 1::s p trace else eval_prog 0::s p trace)
    | x1::[]::[] -> eval_prog [] [] ("Panic"::trace)
    | [] -> eval_prog [] [] ("Panic"::trace))
  | Eq::p ->                     (* = *)
    (match stack with 
